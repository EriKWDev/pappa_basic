/*
 * MG monitor. Copyright Martin Gren 890918
 *
 * Last update: 900415
 *
 * This monitor may be extended to support other commands etc.
 *
 * If this is done, it must maintain the general syntax as defined here.
 *
 * If you do any changes/improvements, please make a log in this file.
 * All suggestions for other improvements may be mailed to martin\@axis.se
 *
 */
...
/*
 *
 * This variable is defined statically for the whole package.
 * If used in a target system, it should be the full system memory.
 *
 */
...
struct comtype  commands[MAXCOMMAND]=
                  {
                    {"hd",do_hd},      {"info",do_info},
                    {"program",do_prog},   {"move",do_move},
                    {"exit",do_exit},      {"fill",do_fill},
                    {"verify",do_verf},    {"check",do_check},
                    {"find",do_find},      {"modify",do_modify},
                    {"load",do_load},      {"save",do_save},
                    {"reg",do_reg},        {"set",do_set},
                    {"run",do_run},
                    {"mode",do_mode},      {"list",do_list},
                    {"help",do_help},      {"reset",do_reset},
                    {"comp",do_comp}
                  };
  ...
  while(bryt == 0)
  {
    ...
    switch(ch)
    {
      ...
      case '-' : x--;
                 break;
      case 'J' :
      case 'j' : memw(x, (getbyt() - x + 2) & 0xff);
                 break;
      case '"' : for(i = 1; inbuf[i] != CR;)
                     memw(x++, inbuf[i++]);
                 break;
      case CR  : break;
      case 46  : bryt = 1;
                 break;
      default  : while((i = getbyt()) != -1)
                     memw(x++, i);
                 break;
    }
  }
  ...
  while(*prg > 0)
  {
     ...
     switch(*prg)
     {
       ...
       case FOR:
            ...
            stack[stack_pnt] = next;
            i = expr(&prg);
            data_stack[stack_pnt++] = i;
            if (*prg == STEP)
            {
              *prg++;
              data_stack[stack_pnt++] = expr(&prg);
            }
            else
              data_stack[stack_pnt++] = 1;
            break;
       case NEXT:
            prg++;
            var = *prg - 'A';
            ...
            j = vars[var];
            tmp = data_stack[stack_pnt * 2 - 2];
      /*      printf("Next var %c  is %d  limit %d\n", var+'A', j, i); */
            if (j <= i)
              next = stack[stack_pnt - 1];
            else
              stack_pnt--;
            break;
       case LET:
...
/*
 * Handle implicit let assignment
 */
assign(prg)
